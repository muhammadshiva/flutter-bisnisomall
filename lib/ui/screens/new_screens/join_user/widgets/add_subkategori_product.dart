...
class AddSubkategoriProduct extends StatelessWidget {
  const AddSubkategoriProduct({
    Key key,
    @required this.name,
    this.categories, this.checkSubCategory,
  }) : super(key: key);

  final String name;
  final List<Subcategory> categories;
  final Function(String value) checkSubCategory;
            ...
            child: ListView.separated(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  ...
                  onTap: () {
                    context.read<AddProductSupplierCubit>().addSubKategori(items[index].name, items[index].id);
                    checkSubCategory(items[index].name);
